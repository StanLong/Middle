...
- 重新启动Jenkins容器后，由于Jenkins需要下载大量内容，但是由于默认下载地址下载速度较慢， 需要重新设置下载地址为国内镜像站。
  ...
  ```shell
  # 进入 /var/jenkins_home/ 挂载的本地目录
  cd /opt/docker/jenkins_docker/data
  ...
  2.319.1-lts 版本的 jenkins 有个插件版本和jenkins不匹配导致产生的坑，readme.md 文件里有记录

- 配置好国内镜像站后再次重启Jenkins容器，访问Jenkins（需要稍微等会）

- 查看密码登录Jenkins，登录并下载插件
...
- 下载Sonar-scanner：https://binaries.sonarsource.com/Distribution/sonar-scanner-cli/ 
  ...
  或者官网下载地址： https://docs.sonarsource.com/sonarqube-server/analyzing-source-code/scanners/sonarscanner
...
- 执行命令检测代码
  ...
  ```shell
  # 项目路径
  [root@node01 mytest]# pwd
  /opt/docker/jenkins_docker/data/workspace/mytest
  
  # 在该路径下执行
  /opt/docker/jenkins_docker/data/sonar-scanner/bin/sonar-scanner \
   -Dsonar.sources=./ \
   -Dsonar.projectname=linux-test \
   -Dsonar.login=a09ffaee6bc34f8df0454a74c966de5f00e59d0e \
   -Dsonar.projectKey=linux-test \
   -Dsonar.java.binaries=./target/
   
  # 参数说明
  -Dsonar.sources  --源码路径
  -Dsonar.projectname --指定检测的项目名称，可以自定义
  -Dsonar.login -- 从sonarqube 我的账号-》安全-》生成令牌
  -Dsonar.projectKey -- 项目标识，建议和项目名称一样
  -Dsonar.java.binaries -- 告诉 sonarqube 编译好的代码路径
  ```
